...
def create_inputs(im, im_info, model_arch='YOLO'):
    ...
    if 'YOLO' in model_arch:
        image = np.array([origin_shape]).astype('float32')
        #image = np.append(3,image)
        im_shape = np.array([resize_shape]).astype('float32')
        #im_shape = np.append(3,im_shape)
        scale_factor = np.array([scale_y,scale_x]).astype('float32')
        #inputs['image'] = image
...
def load_predictor(model_dir,
    ...
    config = inference_config(
        os.path.join(model_dir, 'model310.pdmodel'),
        os.path.join(model_dir, 'model310.pdiparams'))
    if use_gpu:
